...
import com.burgan.kerem.XMLToJava.command.ProcessStoppedCommand;
...
public class FileSplitter extends AbstractObserver {
	...
	public void splitFile() {
		...
		while (totalRecordCount - latestRecordCount > 0 && processing) {
			...
			for (long i = 0; i < divideCnt; i++) {
				...
				if (!processing) {
					...
					getSubject().pickCommand(new ProcessStoppedCommand());
					break;
				}
			...
			//Burada command list içerisine ilgili parametrelerde commandlar yükleniyor.
			getSubject().pickCommand(new FileSenderCommandFactory(splittedFile).getCommand("ftp"));
		...
		closeFiles();
		getSubject().pickCommand(new TextSenderCommandFactory("Processing finished SMS message").getCommand("sms"));
//		getSubject().pickCommand(new TextSenderCommandFactory("Processing finished").getCommand("email"));
